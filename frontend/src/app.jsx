...
function App(){
 ...
 const login=async(email,password)=>{const r=await api.post('/auth/login',{email,password});localStorage.setItem('sc_token',r.data.token);const u={id:r.data.id,name:r.data.name,email:r.data.email,role:r.data.role,department:r.data.department,schoolName:r.data.schoolName,campusName:r.data.campusName};localStorage.setItem('sc_user',JSON.stringify(u));setUser(u);setApiOnline(true);await loadCases();await refreshNotifications();return u};
 const register=async(name,email,password,schoolName,campusName)=>{const r=await api.post('/auth/register',{name,email,password,schoolName,campusName});localStorage.setItem('sc_token',r.data.token);const u={id:r.data.id,name:r.data.name,email:r.data.email,role:r.data.role,department:r.data.department,schoolName:r.data.schoolName,campusName:r.data.campusName};localStorage.setItem('sc_user',JSON.stringify(u));setUser(u);setApiOnline(true);return u};
